...
final class HoverTests: XCTestCase {
   func test_OnHover_FiresOnFocusEnterAndExit() throws {
       var events: [Bool] = []

       let view = VStack {
           Button("One") {}
           Button("Two") {}
               .onHover { events.append($0) }
       }
       ...
       // Focus should start on the first button (no hover event yet)
       ...
       let secondWrapper = try XCTUnwrap(outer.children.dropFirst().first) // our onHover wrapper
