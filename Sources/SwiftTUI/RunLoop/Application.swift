   ...
   private func handleInput() {
       ...
       for char in string {
           ...
           if let key = arrowKeyParser.arrowKey {
               ...
               if !moved {
                   // No focus movement possible; forward arrow intent to the focused control
                   // using a vim-key mapping so controls can react (e.g., Picker, custom controls).
                   // Avoid remapping arrows to letters when a text input is focused.
