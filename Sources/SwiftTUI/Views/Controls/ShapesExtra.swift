...
public struct Capsule: View, PrimitiveView {
 ...
 class CapsuleControl: Control {
     ...
     static func containsCapsule(_ pos: Position, size: Size) -> Bool {
         ...
         let centerY = h / 2.0
         if w >= h {
             // Horizontal capsule (floored radius to avoid including extreme corners)
             let r = floor(h / 2.0)
             let leftCenterX = r
             ...
             // Vertical capsule (floored radius to avoid including extreme corners)
             let r = floor(w / 2.0)
             let centerX = w / 2.0
             let topCenterY = r
...
public struct Ellipse: View, PrimitiveView {
  enum Mode { case fill(Color), stroke(Color) }
  private var mode: Mode? = nil

  public init() {}

  public func fill(_ color: Color) -> Ellipse { var c = self; c.mode = .fill(color); return c }
  public func stroke(_ color: Color) -> Ellipse { var c = self; c.mode = .stroke(color); return c }

  static var size: Int? { 1 }

  func buildNode(_ node: Node) { node.control = EllipseControl(mode: mode) }
  func updateNode(_ node: Node) { node.view = self; (node.control as? EllipseControl)?.mode = mode; node.control?.layer.invalidate() }
  ...
  class EllipseControl: Control {
      ...
      override func cell(at position: Position) -> Cell? {
          guard position.column >= 0, position.line >= 0,
                position.column < layer.frame.size.width,
                position.line < layer.frame.size.height else { return nil }
          func contains(_ pos: Position) -> Bool { EllipseControl.containsEllipse(pos, size: layer.frame.size) }
          switch mode {
          case .fill(let c):
              return contains(position) ? Cell(char: " ", backgroundColor: c) : nil
          case .stroke(let c):
      ...
      static func containsEllipse(_ pos: Position, size: Size) -> Bool {
          let w = Double(size.width.intValue)
          let h = Double(size.height.intValue)
          if w <= 0 || h <= 0 { return false }
          let cx = Double(pos.column.intValue) + 0.5
          ...
          let rx = w / 2.0
          let ry = h / 2.0
          let dx = (cx - centerX) / max(rx, 0.0001)
          let dy = (cy - centerY) / max(ry, 0.0001)
