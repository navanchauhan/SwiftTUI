...
/// - Tabs can be specified via a `titles` array or derived from child `.tabItem(title:)` modifiers.
/// - Content should provide the same number of child views as there are tabs.
...
public struct TabView<Content: View>: View, PrimitiveView, LayoutRootView {
   // If provided, use these titles; otherwise derive from child .tabItem(title:)
   let providedTitles: [String]?
   ...
   public init(titles: [String], selection: Binding<Int>, @ViewBuilder content: () -> Content) {
       self.providedTitles = titles
       self.selection = selection
       self.content = VStack(content: content())
   }

   /// Create a TabView that derives titles from child views via `.tabItem(title:)`.
   /// - Parameters:
   ///   - selection: Index of the currently selected tab.
   ///   - content: Child views; each may set its title via `.tabItem(title:)`.
   public init(selection: Binding<Int>, @ViewBuilder content: () -> Content) {
       self.providedTitles = nil
       self.selection = selection
       self.content = VStack(content: content())
   }
   ...
   func buildNode(_ node: Node) {
       ...
       let c = TabViewControl(titles: providedTitles ?? [], selection: selection)
   ...
   private func refreshPages(node: Node, control: TabViewControl) {
       ...
       var derivedTitles: [String] = []
       if let builderNode = vStackNode.children.first {
           for i in 0 ..< builderNode.size {
               pageControls.append(builderNode.control(at: i))
               // Try to read the title from the child's environment
               let childNode: Node = (i < builderNode.children.count) ? builderNode.children[i] : builderNode
               let envTitle: String? = TabView.readEnvironment(\._tabItemTitle, from: childNode)
               derivedTitles.append(envTitle ?? "Tab \(i+1)")
           }
       }
       control.allContentControls = pageControls
       // Prefer provided titles; otherwise use derived ones
       if let provided = providedTitles {
           control.titles = provided
       } else {
           control.titles = derivedTitles
       }
       control.rebuildTabBar()
       control.installSelectedContent()
   }
