...
private class ScrollControl: Control {
    ...
    override func layout(size: Size) {
        ...
        // Measure content with a viewport-aware proposal so it can compute its
        // natural extent on the scrolling axis while respecting the cross-axis.
        let proposal: Size
        switch axis {
        case .vertical:
            // Constrain width to viewport; allow vertical to expand.
            proposal = Size(width: size.width, height: .infinity)
        case .horizontal:
            // Constrain height to viewport; allow horizontal to expand.
            proposal = Size(width: .infinity, height: size.height)
        }
        let contentSize = contentControl.size(proposedSize: proposal)
    ...
    @discardableResult
    override func scrollBy(lines: Int = 0, columns: Int = 0) -> Bool {
        var changed = false
        switch axis {
        case .vertical:
            if lines != 0 {
                let viewport = layer.frame.size.height.intValue
                let content = contentControl.layer.frame.size.height.intValue
                if viewport > 0 && content > viewport {
                    let maxScroll = content - viewport
                    let current = contentOffset.intValue
                    let next = max(0, min(maxScroll, current + lines))
                    if next != current {
                        contentOffset = Extended(next)
                        // Reposition content for immediate effect
                        contentControl.layer.frame.position.line = -contentOffset
                        contentControl.layer.frame.position.column = 0
                        layer.invalidate()
                        changed = true
        ...
        case .horizontal:
            if columns != 0 {
                let viewport = layer.frame.size.width.intValue
                let content = contentControl.layer.frame.size.width.intValue
                if viewport > 0 && content > viewport {
                    let maxScroll = content - viewport
                    let current = contentOffset.intValue
                    let next = max(0, min(maxScroll, current + columns))
                    if next != current {
                        contentOffset = Extended(next)
                        contentControl.layer.frame.position.column = -contentOffset
                        contentControl.layer.frame.position.line = 0
        ...
        case .vertical:
            ...
            if position.column == viewport.width - 1 {
                ...
                if line >= start && line < start + length {
                    return Cell(char: "│", foregroundColor: sc.accentColor)
                }
            }
            return nil
        case .horizontal:
