...
class Control: LayerDrawing {
  ...
  func becomeFirstResponder() {
      scroll(to: .zero)
      parent?.descendantBecameFirstResponder(self)
  }

  func resignFirstResponder() {
      parent?.descendantResignedFirstResponder(self)
  }
  ...
  // Manual scroll by delta. Returns true if a scrollable ancestor handled it.
  // Default implementation forwards to the parent until a scroll-capable
  // control (e.g., ScrollView) overrides it.
  @discardableResult
  func scrollBy(lines: Int = 0, columns: Int = 0) -> Bool {
      return parent?.scrollBy(lines: lines, columns: columns) ?? false
  }

  // MARK: - Focus notifications (bubble up)

  /// Called on ancestors when a descendant becomes first responder.
  /// Default implementation bubbles up to parent.
  func descendantBecameFirstResponder(_ control: Control) {
      parent?.descendantBecameFirstResponder(control)
  }

  /// Called on ancestors when a descendant resigns first responder.
  /// Default implementation bubbles up to parent.
  func descendantResignedFirstResponder(_ control: Control) {
